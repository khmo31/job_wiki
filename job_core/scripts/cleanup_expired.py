...
def _get_raw_value(raw: dict, key: str) -> str:
    """Read a value from raw data, supporting both nested (raw.list) and flat formats."""
    if not isinstance(raw, dict):
        return ""
    # 1) nested format: raw.list (ALIO API merged data)
    raw_list = raw.get("list", {})
    if isinstance(raw_list, dict) and raw_list.get(key):
        return str(raw_list[key])
    # 2) flat format (legacy / direct storage)
    if raw.get(key):
        return str(raw[key])
    return ""
...
def _get_recrut_pblnt_sn(archive: dict) -> str | None:
    ...
    sn = _get_raw_value(raw, "recrutPblntSn")
...
def _get_close_date(archive: dict) -> date | None:
    ...
    Supports both nested (raw.list) and flat raw formats.
    """
    raw = archive.get("raw", {}) if archive else {}
    end_ymd = _get_raw_value(raw, "pbancEndYmd")
    return _parse_date_str(end_ymd) if end_ymd else None


def _get_company_name(archive: dict) -> str:
    raw = archive.get("raw", {}) if archive else {}
    inst_nm = _get_raw_value(raw, "instNm")
    if inst_nm:
        return inst_nm
    # Fallback: check direct company field on archive
    return archive.get("company", archive.get("instNm", "")) or ""
...
def main() -> int:
    ...
    for alio_id, entry in index.items():
        ...
        # DEBUG: show close date extraction
        if archive:
            raw = archive.get("raw", {}) if archive else {}
            raw_list = (raw.get("list", {}) if isinstance(raw, dict) else {}) if isinstance(raw, dict) else {}
            pbanc_flat = raw.get("pbancEndYmd", "(none)") if isinstance(raw, dict) else "(none)"
            pbanc_nested = raw_list.get("pbancEndYmd", "(none)") if isinstance(raw_list, dict) else "(none)"
            _log(f"  [{alio_id}] pbancEndYmd: flat={pbanc_flat}, nested.list={pbanc_nested}, parsed={close_date}")
    ...
    _log(f"scan complete: {total} total, {no_close_date} no-close-date, {len(expired_ids)} expired")
